...
?>
    ...
    ?> 
            <div class="row">
              ...
              if($show_details==1){
                    $query=  $this->db->query("SELECT e.id,sub.sid,sub.subject,e.`examdate`,e.`starttime`,e.`endtime`,e.`timespan`,e.`maxmarks`,e.`minmarks` FROM `exam` e JOIN subjects sub ON e.subid=sub.sid JOIN examination_cls ec ON e.ecid=ec.id  WHERE ec.`id` ='".$section_query->id."'  ORDER BY sub.sid asc");
                    $query=$query->result();
                    $days =array();
                    ...
                    foreach($query as $val){
                        $exam_settings[$val->sid]=array('exam_id'=>$val->id,'date'=>$val->examdate,'starttime'=>$val->starttime,'endtime'=>$val->endtime,'timespan'=>$val->timespan,'maxmarks'=>$val->maxmarks,'minmarks'=>$val->minmarks);
                    }
                    ?>
            <div class="box">
                <hr/>
                <form class='form-horizontal form-bordered' action="<?php echo base_url(); ?>index.php/exams/savessettings" method="post" enctype="multipart/form-data"  >
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <td>
                                           ...
                                           $end_time ="";
                                              if(strlen($this->form->value('ending_'.$value->cid))>0 ){
                                                   $end_time= $this->form->value('ending_'.$value->cid);
                                                }else{
                                                    if(isset($exam_settings[$value->sid])){
                                        ...
                                        <input type="hidden"  <?php
                                          ...
                                          ?> placeholder="max Marks"  name="max_<?php echo $value->cid ?>" min="0" class="form-control" value="20"   />
                                            <input type="hidden"
                                                   <?php
                                          if(strlen($this->form->error('min_'.$value->cid))>0 ){
                                                 echo  ' style="border:1px solid #FF0000"';
                                          }
                                          ?>  placeholder="Cut-off Marks"  name="min_<?php echo $value->cid ?>" min="0" class="form-control" value="6"  />
                                </tr>
                                      <?php
                                  }
                ...
                <div class="col-sm-4 nopadding" >
                    <div style=" padding-left: 20px;" class="box box-bordered box-color ">
                        <div class="box-title">
                            <h3><i class="fa fa-cogs"></i>Summative Settings</h3>
                        </div>
                        <div class="box-content nopadding">
                            <table class="table table-bordered" >
                                <input type="hidden" name="max_marks" class="form-control" value="80" /> 
                                <tr>
                                    <th>Min Marks</th>
                                    <td><input type="text" name="min_marks" class="form-control" value="<?php
                                          if(strlen($this->form->error('min_marks'))>0 ){
                                                 echo $this->form->error('min_marks') ;
                                          }else{
                                              echo "40";
                                          }
                                          ?>" /></td>
                                ...
                                <tr>
                                    <th>Max Marks</th>
                                    <td ><span class="form-control" >80</span></td>
                                </tr>
                                <?php
                                    $e_Q="SELECT e.exam  FROM `examination_cls` c JOIN examinations e ON c.examid=e.id  where c.sectionid=".$section_query->sid." and c. status=1 AND e.type=4 AND e.startdate<".$exam->startdate."";
                                    $e_Q=$this->db->query($e_Q)->result();
                                    $e="";
                                    foreach($e_Q as $v){
                                     $e.=$v->exam."  <br/>,";   
                                    }
                                ...
                                ?>
                                <tr>
                                    <th>Average  Of <br/><?php echo $e ?></th>
                                    <td><span class="form-control" >20</span></td>
                                </tr>
                                <tr>
                                    <th>Total Marks</th>
