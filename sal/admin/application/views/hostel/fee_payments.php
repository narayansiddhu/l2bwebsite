        ...
        <div class="box">
            ...
            if(strlen($this->session->userdata('stud_fee_payee_Sucess'))>0 ){
                ?><br/>
                    ...
                    Fee Payed Successfully , Invoice No : <?php echo $this->session->userdata('stud_fee_payee_Sucess'); ?>  , <a target="_blank" href="<?php echo base_url() ?>index.php/Hostel/Invoice/<?php echo $this->session->userdata('stud_fee_payee_Sucess'); ?>">Click Here for invoice</a>  
                    </div>
                    <script>
                             $("#successMsg").fadeIn();
            ...
            <div class="box" style=" height: auto;" >
                <div class="col-sm-4 nopadding">
                    <div class="box box-bordered box-color" style=" padding-right:  2px;">
                        <div class="box-title">
                                <h3><i class="fa fa-money"></i>Hostel Fee Payments</h3> 
                        </div>
                        <div class="box-content nopadding">
                            <?php
                               $stud="SELECT st.hsid,st.fee ,s.student_id,s.name ,s.phone,s.userid FROM hostel_students st JOin student s On st.student_id= s.student_id  where st.iid='".$this->session->userdata('staff_Org_id')."' ";
                                            ...
                                            <select onchange="load_balance();"  name="student" id="student" class='select2-me' style="width:100%;" >
                                                <option value="">Select A student</option>
                                                 <?php
                                                  foreach($stud as $val){
                                                       ?>
                                                 <option value="<?php echo $val->student_id.",".$val->fee.",".$val->student_id ?>" >
                ...
                function load_balance(){
                    ...
                    if(student.length==0){
                        ...
                        setState('fee_balance','<?php echo base_url() ?>index.php/Hostel/load_balance','student='+student[0]+'&fee_amount='+student[1]);
                ...
                function Add_trans_fee(){
                    ...
                    if(count==0){
                       student = student.split(','); 
                      // alert(student[0]);
                       setState('errors','<?php echo base_url() ?>index.php/Hostel/add_fee','student='+student[2]+'&payee='+payee+'&total_paid='+$('#fee_paid').html()+'&fee_amount='+student[1]);
                    ...
                    <div class="box box-bordered box-color" style=" padding-left:  15px;">
                        ...
                        <div class="box-content nopadding">
                            <table class="table table-bordered datatable" style=" width: 100%; " > 
                                ...
                                <tbody>
                                    <?php
                                     $tfee=" SELECT tf.*,s.name , s.userid FROM `hostel_feepayment` tf JOIN student s  On tf.student= s.student_id   WHERE tf.iid='".$this->session->userdata('staff_Org_id')."' ";
