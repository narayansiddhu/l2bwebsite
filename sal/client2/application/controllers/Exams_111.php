...
<h3 style=" text-align: center; color:  #009999; font-size: 18px ">Schedule Of <?php echo $exam->exam ?></h3>
	...
	public function print_summative_result($eid,$section) {
        if($eid ==0){
           redirect('exams/index','refresh'); 
        }else{
            $exam=$this->examinations->getexam_data($eid);
...
<hr color="#00306C" />';
			...
			?>
			<table align="center" class="tab_td1"  style="width:100%" >
                   <thead>
                       <tr>
                           <th>Student</th>
                           <?php
...
$institute =  $this->fetch_institute_details();
                          ...
                          echo '<br/><br/><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Exam Result</title>
...
.total {width:100%;margin-left:auto;padding:1px; margin-right: auto; }
.right_txt p{padding:0px; margin:0px; text-align:justify; color:#407c3d; text-align:right; line-height:1.4em; }
p{ text-align:justify;}
.left{float:left; padding:0px; margin:0px;}
.right{float:right;padding:0px; margin:0px; }
...
.marks{border:0.1px solid #000;   }
.marks  td{ text-align:center;  padding:1px; border-right:0.3px solid #000 ; vertical-align:text-top; }
.marks  th{ text-align:center;  padding:1px; border-right:0.3px solid #000 ; vertical-align:text-top;background-color:#E3E3E3; }
...
<!--                            <th>No</th>-->
                        ...
                        <tbody>
                            ...
                            <tr>
<!--                                <td><?php echo $value->roll ?></td>-->
                                <td style=" font-size: 9px ; font-weight:  bold"><?php echo $value->name ?></td>
                                <?php
                                foreach ($query as $sub){
                                    ?>
                                <td>
                                        <?php 
                                         if(isset($maarks_array[$sub->id][$value->id])){
                                             ?>
                                        <?php echo $maarks_array[$sub->id][$value->id]['part1'] ?>|<?php echo $maarks_array[$sub->id][$value->id]['part2'] ?>|<?php echo $maarks_array[$sub->id][$value->id]['part3'] ?>|<?php echo $maarks_array[$sub->id][$value->id]['part4'] ?><br/><?php echo $maarks_array[$sub->id][$value->id]['total'] ?>|<?php echo $maarks_array[$sub->id][$value->id]['Grade'] ?>
                                             <?php
                                         }else{
                                             ?>
                                              <?php
                                ...
                                ?>
                                <td style=" text-align: left"><?php echo $stud_marks[$value->id]['total_marks'] ?><br/><?php echo $grade_array[1]['grading'][$stud_marks[$value->id]['grade']]['Grade_points']; ?>/<?php echo $grade_array[1]['grading'][$stud_marks[$value->id]['grade']]['grade']; ?>/<?php echo  find_pos($totals, $stud_marks[$value->id]['total']); ?></td>
                            </tr>    
                                    <?php
                                }
                            ?>
                ...
                foreach($s as $stud){
                    ...
                    $stud_details='<br><br>
                                    <table cellpadding="0" cellspacing="0" width="100%" border="0" bgcolor="#FFFFFF">
                                        ...
                                        <td align="center"><!--<img src="#" width="125" height="92" />--></td>
                                        <td align="right">
                                        <div class="right_txt" style="color:#00306C"><h1 style="font-size:22px;font-weight: bold ">'.strtoupper($institute->name).'</h1>
                                        </div>
                                        </td>
                                        </tr>
                                        </table>
                                        <hr noshade>
                                    <div class="studet_card">
                                  <table align="left" class="tab_td2" border="0" width="100%" cellpadding="0" cellspacing="0">
                                    <tr>
...
';
                                ...
                                <div  style="width:33%;float:left;text-align:center" class="print"><br/><br/>
                                <span style="border-top:1px solid #000000;text-align:center;font-size:14px">Principal Sign</span>
                                </div> ';
...
<script>/*var css = ' @page {margin-top:45mm; .print:last-child {page-break-after: auto;} }',
...
window.print();</script>
    ...
    public function print_formative_cards1($exam_id="",$section_id="",$student=""){
                if( (strlen($exam_id)!=0) &&(strlen($section_id)!=0) ){
            $exam=$this->examinations->getexam_data($exam_id);
        if(!$exam){
             redirect('exams/index','refresh');
            ...
            $t=$section = $section_id;
             $grade_array=  unserialize(FMA_GRADING);
            $course_err ="";
            if( strlen($t)!=0 ){
                $section_query=$this->db->query("SELECT ec.id,s.sid,s.name as section,c.name as class ,c.medium FROM `examination_cls` ec JOIN section s ON ec.sectionid=s.sid JOIN class c ON s.class_id=c.id where ec.examid='".$exam->id."' AND ec.status=1 AND s.sid='".$t."' ");
              if($section_query->num_rows()==0){
 ...
 $marks= $this->db->query($marks)->result();
  ...
  $t=0;
        ...
        $value->marks=gpa_grade($value->marks, $grade_array);
        $ranks[$value->exam_id][$value->marks]=$value->marks;
       ...
       $maarks_array[$value->exam_id][$value->student_id]= array(
           'total'=>$t,
           "Grade"=>$grade_array[$value->marks]['grade'],
               );
      //  $stud_marks[$value->student_id]['total']+=$grade_array[$subject_array[$value->exam_id]['type']]['grading'][$value->marks]['Grade_points'];              
    //total_marks
        $stud_marks[$value->student_id]['total_marks']+=$t;
...
body {margin-left: 0px;margin-top: 0px;margin-right: 0px;margin-bottom: 0px; font-size:10px;}
.total {width:100%;margin-left:auto;padding:5px; margin-right: auto; clear:both; }
.studet_card {width:50% ; float: left; margin-left:auto;padding:5px; margin-right: auto; min-height:700px; border-bottom : 1px solid black}
.right_txt p{padding:0px; margin:0px; text-align:justify; color:#407c3d; text-align:right; line-height:1.4em; }
p{ text-align:justify;}
.left{float:left; padding:0px; margin:0px;}
.right{float:right;padding:0px; margin:0px;	}
...
.tab_td1  th{ text-align:center;border:1px solid #000; padding:5px 0px;background-color:#E3E3E3; }
 ...
 .tab_td2{border:0px solid #000; margim-bottom:150px;font-size:14px;  }
.tab_td2  td{ text-align:left; border:0px solid #000; padding:4px 2px; vertical-align:text-top; }
.tab_td2  th{ text-align:left;border:0px solid #000; padding:5px 0px; }
...
$medium=  unserialize(medium);
                ...
                $i=1;
                $grading_str='<h4 style="text-align:left;">Grading Particulars</h4>          
                                        <table style="font-size:10px;" align="center" class="tab_td1" border="0" width="100%" cellpadding="0" cellspacing="0">
<tr>
<th>GPA</th>
<th>Grade</th>
<th>Description</th>
...
';
                        ...
                        $grading_str=$grading_str."</table>";
                foreach($s as $stud){
                    $sty="clear:both;width: 45%;";
                    if($i%2==0){
                        $sty="margin-left:20px;width: 45%;";
                    }
                    $stud_html_str='<div style=" height: 700px; padding-bottom:25px;  padding: 20px;width: 45%;float: left;'.$sty.' border-right: 1px solid #999999">';
                    $stud_html_str.=$institute_html;
                    $i++;
                    $stud_details='<hr/>
                                  <table align="left" class="tab_td2" border="0" width="100%" cellpadding="0" cellspacing="0">
                                    <tr>
                                        <td style="text-align:left" >Student Name :<span style="font-weight: bold;"> '.strtoupper($stud->name).'</span></td>
                                        <td  style="text-align:right" >Class : '.$section_query->class.'</td>
                                        <td  style="text-align:right" >Roll No : '.$stud->roll.'</td>
                                    </tr>
                                    </table>
                    ...
                    $stud_html_str.=$stud_details;
                         $subjects_marks='<div style="border:2px solid #00306C;text-align:center;background-color:#A4A4A4;">
                                        ...
                                        <table style="font-size:13px;" align="left" class="tab_td1" border="0" width="100%" cellpadding="0" cellspacing="0">
                                        <tr>
                         ...
                         foreach ($query as $value) {
                             ...
                             $subjects_marks.='<tr>
                                             <td width="13%">'.strtoupper($value->subject).'('.$value->maxmarks.') </td>
                                            <td width="13%">'.$gra.'</td>
                                            <td width="13%">'.number_format((($total/$value->maxmarks)*100)/10,1).'</td>
                                        </tr>';
                         }
                        $stud_html_str.=$subjects_marks."</table>";
                         $mont_html='<th width="60%" >Total No Of Working Days</th><td>&nbsp;</td>';
                        $mn_per_html="<th width='60%' >Total No of Present Days</th><td>&nbsp;</td>";
//                        foreach($att_month_query as $att=>$per){
//                            
//                            $mont_html.="<th>".strtoupper($att)."</th>";
//                            $mn_per_html.="<td>".( number_format(($stud_marks[$stud->id]['attendance'][$att]/$per['count'])*100,2) )."%</td>";
...
';
                        ...
                        $stud_html_str.=$attendance_html.$signs.$grading_str."</div>";
                         echo $stud_html_str;
...
//                print_r($data);
	...
	public function summative_report_card($eid="",$section="",$student=""){
...
$query=$query->result();
        ...
        $studprev_marks_array=array();
        $stud_marks=array();
        $s=$this->db->query("SELECT student_id as id,name,roll from student WHERE section_id ='".$section_query->sid."'  ")->result();;
...
}else
{
  ...
  function gpa_grade($mark,$gpa_array){
	  $gpa_array=unserialize(FMA_GRADING);
                $mark=  number_format($mark);
                foreach ($gpa_array as $key => $value) {
                   if( ($mark>=$value['from'])&&($mark<=$value['to'])  ){
                      return $key; 
  ...
  foreach($s as $stud){
    foreach($query as $sub){
       ...
       if($stud_marks[$stud->id][$sub->sid]['sm']!="A"){
        ...
        $total=$sm+$fm;$total=$sm+$fm;
        $stud_marks[$stud->id][$sub->sid]['total']=$sm+$fm;
		 $stud_marks[$stud->id][$sub->sid]['grade']=$grade_array[gpa_grade(($sm+$fm), $grade_array)]['grade'];
  ...
  //print_r($stud_marks);exit;
   foreach ($ranks as $key => $value) {
    ...
    foreach($stud_marks as $k=>$value){
        ...
        $stud_marks[$k]['grade']=gpa_grade(round($value['total']/$overall_total*100), $grade_array);
      if(isset($grade_graph_array[$stud_marks[$k]['grade']])){
           $grade_graph_array[$stud_marks[$k]['grade']]++;
      }
       $totals[$value['total']]=$value['total'];
...
body {height:100%;margin-left: 0px;margin-top: 0px;margin-right: 0px;margin-bottom: 0px; font-size:10px;}
.total {width:100%;margin-left:auto;padding:5px; margin-right: auto; clear:both; }
.studet_card {width:50% ; float: left; margin-left:auto;padding:5px; margin-right: auto; min-height:700px; border-bottom : 1px solid black}
.right_txt p{padding:0px; margin:0px; text-align:justify; color:#407c3d; text-align:right; line-height:1.4em; }
p{ text-align:justify;}
...
';        
        $html_final='';
        $institute_html='
<table cellpadding="0" cellspacing="0" width="100%" border="0" bgcolor="#FFFFFF" border="1px solid red">
<tr><td align="left"><img src="'. assets_path ."/uploads/".$institute->logo.'" width="118" height="28px" /></td>
<td align="center"><!--<img src="#" width="125" height="72" />--></td>
<td align="right">
<div class="right_txt" style="color:#00306C"><h1 style="font-size:22px;font-weight: bold ">'.strtoupper($institute->name).'</h1>
</div>
</td>
...
$medium=  unserialize(medium);
                //$overall_total=0;
                                $att_month_query=array();
                                $att_query=$this->db->query("SELECT day,group_concat(id) as attd_id FROM `attendance_date` where section='".$section."'  GROUP BY day ORDER BY day ASC");
                                $att_query=$att_query->result();
                                $prev_month="";
                                foreach($att_query as $at){
                                    $day  = getdate($at->day);
...
$grade_array=unserialize(FMA_GRADING);
		...
		foreach($s as $stud){
                    ...
                    $stud_html_str='<div style=" height: 700px; padding-bottom:25px;  padding: 20px;width: 45%;margin-top:20px;float: left; '.$sty.' border-right: 1px solid #999999">';
                    $stud_html_str.=$institute_html;
                    $i++;
                    $stud_details='<hr/>
                                  <table align="left" class="tab_td2" border="0" width="100%" cellpadding="0" cellspacing="0">
                                    ...
                                    ';
                    $stud_html_str.=$stud_details;
                         $subjects_marks_html='<div style="border:2px solid #00306C;text-align:center;background-color:#A4A4A4;">
                                        <h2>Result of SUMMATIVE- 2</h2>
                                        </div>
                                        <table style="font-size:13px;" align="left" class="tab_td1" border="0" width="100%" cellpadding="0" cellspacing="0">
                                        <tr>
                                            <th width="50%">Subject</th>
                          ...
                          foreach ($query  as $sub) {
                             $sm=$fm=$total=$gra="--";
										...
										if(isset($stud_marks[$stud->id][$sub->sid]['grade'])){
                                          $gra= $stud_marks[$stud->id][$sub->sid]['grade'];  
                                        }else{
                                          $gra="0";
                                        }
										//$stud_marks[$stud->id][$sub->sid]['grade']
                                        $total= $sm+$fm;
                                
                             $subjects_marks_html.='<tr>
							 				
                                             <td width="13%">'.strtoupper($sub->subject).'</td>
											  <td width="13%"></td>
											   <td width="13%"></td>
											   <td width="13%"></td>
                                            <td width="13%"></td>
						 ...
						 $subjects_marks_html.=' <table style="font-size:13px;" align="left" class="tab_td1" border="0" width="100%" cellpadding="0" cellspacing="0">
						 <tr>
						 <th width="60%">Grand Total</th>
						 <td></td>
						 </tr>
						 <tr>
						 <th width="60%">Total Grade</th>
						 <td></td>
						 </tr>
						 </table>
						 ';
						 $subjects_marks_html.='&nbsp;&nbsp;
						  ...
						  <table style="font-size:13px;" height="0px" align="left" class="tab_td1" border="0" cellpadding="0" cellspacing="0" width="100%">
						 <tr>
						 <th width="60%"  colspan="4">Attendance</th>
						 ...
						 '."<br>";
						  ...
						  <table style="font-size:13px;" align="left" class="tab_td1" border="0" cellpadding="0" cellspacing="0" width="100%">
						 <tr>
						...
						<th >Grade</th>
						 ...
						 <th></th>
						  ...
						  <tr>
						 <th width="60%">Work & Computer Education</th>
						 <th></th>
						 <th></th>
						 ...
						 ';
                        $stud_html_str.=$subjects_marks_html."</table>";
