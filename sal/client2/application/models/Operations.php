<?php  
   ...
   {  
      ...
      public function validate_user($username,$password){
         ...
         public function validated_user($id,$username,$password){
            $this->load->model('logs');
              $credential = array('email' =>$username, 'password' => $password,'status' => '1','category' => $id, 'iid' => $this->session->userdata('staff_Org_id'));
                $query = $this->db->get_where('accounts', $credential);
                if ($query->num_rows() > 0) {

                   $row = $query->row();
                    $query="SELECT *  FROM `accounts` WHERE category= $id";
                    $query = $this->db->query($query);
                    $query= $query->row();
        ...
        public function validate_otp($otp){
                ...
                $query = $this->db->get_where('accounts_otp', $credential);
                if ($query->num_rows() > 0) {
                  $row = $query->row();
                    $this->session->set_userdata('otp', $row->category);
